...
const EquipmentCategoryDetailPage = () => {
  ...
  const getProductById = async () => {
    try {
      ...
      if (result.success) {
        setProduct(result.equipment);
        setSelectedImage(result.equipment.images[0]); // Set the first image as default
        setFormData((prevData) => ({
          ...prevData,
          productDetails: `Hello, I would like to know more about this product, productCode = ${result.equipment.productCode}, productName = ${result.equipment.productName}, productCategory = ${result.equipment.category} !`,
        }));
      } else {
  ...
  useEffect(() => {
    getProductById(); // Fetch the product details when the component mounts
  }, [id]);
    ...
    <>
      ...
      <div className="flex flex-col h-screen">
        ...
        <div className="fixed bottom-5 right-5">
          <WhatsAppIcon
            phoneNumber="9084730912"
            message={formData.productDetails}
          />
        </div>
        <Footer />
      </div>
